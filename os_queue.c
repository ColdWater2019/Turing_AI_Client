...
os_queue_handle_t os_queue_create(size_t item_count, size_t item_size) {
    os_queue_handle_t queue = (os_queue_handle_t)os_calloc(1,sizeof(*queue));
    int i;
    ...
    queue->write_pos = 0;
    queue->state = 0;
    queue->item = os_calloc(item_count,sizeof(char*));
    for(i=0; i<item_count; i++) {
        queue->item[i] = os_calloc(1,item_size);
    }
...
OS_BOOL os_queue_is_full(os_queue_handle_t self) {
    OS_BOOL is_full = false;
    os_mutex_lock(self->lock);
    if(self->fill >= self->item_count) {
        is_full = true;
    }
...
OS_BOOL os_queue_is_empty(os_queue_handle_t self) {
    OS_BOOL is_empty = false;
    os_mutex_lock(self->lock);
    if(self->fill == 0) {
        is_empty = true;
    }
...
void os_queue_destroy(os_queue_handle_t self) {
    if(self) {
        ...
        while(--self->item_count) {
            os_free(self->item[self->item_count]);
        }
        os_free(self);
    }    
}
